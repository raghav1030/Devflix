    ...
    <div className=' w-screen flex items-center justify-center absolute inset-0  ' >
        {
            ...
            <div className=' flex flex-col lg:w-[444px] h-max mx-auto gap-3   '>
                ...
                <p className='text-richBlack-600  text-sm  '>
                    {
                        mailSent ? `We have sent the reset email to ${<span> ${email} </span>}` : "Have no fear. We'll email you the instructions to reset your password. If you don't have access to your email, we can try account recovery. "
                    }
                </p>

                <form action="submit" onSubmit={handleOnSubmit} className=' w-full flex flex-col justify-center items-start gap-4 '  >
                    {
                        !mailSent && (
                            <label className='w-full flex flex-col justify-center items-start gap-3  ' >
                                <p className='text-sm  ' >Email Address</p>
                                <input type="email" name='email' required onChange={(e) => setEmail(e.target.value)} placeholder='Enter Your Email Address' className='w-full px-4 bg-richblack-800 border-b border-richblack-300 text-md text-richblack-5 h-11 rounded-lg ' />
                            </label>
                        )
                    }
