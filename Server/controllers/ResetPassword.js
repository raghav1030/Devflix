  ...
  try {
    ...
    if (!userDetails) {
      return res.status(401).json({
        success: false,
        message:
          "You are not authorised to move further as the token does not match with the token that is saved in your database and linked with this ID",
      });
    }
    ...
    if (userDetails.resetPasswordExpires < Date.now()) {
      return res.status(402).json({
        success: false,
        message:
          "Your token is expired as you took more than 5 minutes. Please retry ",
      });
    }
